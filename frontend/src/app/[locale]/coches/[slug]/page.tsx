...
import type { CarDto } from "@/types/car/carDto";
...
export async function generateMetadata({
  ...
  try {
    ...
    return {
      title,
      description,
      // Sold cars stay reachable (links + recommendations keep their value)
      // but must drop out of the index.
    ...
    // Unknown slug. The page below calls notFound(); Next 16 streams this route
    // so the HTTP status stays 200 (see report: soft-404 open issue), which
    // makes the noindex here the thing that actually keeps it out of the index.
    return {
...
export default async function CarDetailPage({ params }: CarDetailPageProps) {
  ...
  // Only the fetch is guarded: a `notFound()` raised further down must not be
  // swallowed by this catch and turned back into a 200.
  let data: CarDetailsDto;
  let similarCars: CarDto[];
  try {
    [data, similarCars] = await Promise.all([
